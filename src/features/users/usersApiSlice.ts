...
export const usersApiSlice = apiSlice.injectEndpoints({
    endpoints: builder => ({
        getUsers: builder.query<EntityState<IUser>, {}>({
            query: () => ({
                url: '/users',
            ...
            transformResponse: (responseData: IUser[]): EntityState<IUser> => {
        ...
        deleteUser: builder.mutation<EntityState<IUser>, { id: string | undefined }>({
            query: ({ id }) => ({
                url: `/users`,
                method: 'DELETE',
                body: { id }
